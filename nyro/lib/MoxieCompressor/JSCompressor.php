...
class Moxiecode_JSCompressor {
	 ...
	 * @param bool $whitespace Set this state to false to skip whitespace removal for the specified content.
	 */
	function addContent($content, $whitespace = true) {
		$this->_items[] = array('content', $content, $whitespace);
	}
	...
	function compress() {
		...
		if (!isset($_GET["gz"]) && $supportsGzip && $this->_settings['patch_ie'] && strpos($_SERVER["HTTP_USER_AGENT"], "MSIE") !== false) {
			// Build request URL
			$url = $_SERVER["REQUEST_URI"];  // Updated by Nyro
