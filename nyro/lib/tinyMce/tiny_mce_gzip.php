...
if (TinyMCE_Compressor::getParam("js")) {
	 ...
	 *  "expires"   => "1m",
	 */
		"cache_dir" => defined('TINYMCECACHEPATH')? TINYMCECACHEPATH : realpath("."), // Cache path, this is where the .gz files will be stored /////////////////////////// Update for nyroFwk ///////////////////////////
	));
...
class TinyMCE_Compressor {
	...
	public function handleRequest() {
		...
		$tinymceDir = TINYMCEPATH;/////////////////////////// Update for nyroFwk ///////////////////////////
		$useMin = DEV ? '' : '.min';
		...
		foreach ($plugins as $plugin) {
			$files[] = "plugins/$plugin/plugin".$useMin;
		...
		foreach ($themes as $theme) {
			$files[] = "themes/$theme/theme".$useMin;
		...
		for ($i = 0; $i < count($allFiles); $i++) {
			...
			if ($this->settings["source"] && file_exists($tinymceDir.$file . "_src.js")) {
				$file .= "_src.js";
			} else if (file_exists($tinymceDir.$file . ".js"))  {
				$file .= ".js";
			} else {
				$file = "";
			}
